%----------------------------------------------------------------------
% The code here is Model 1 for Os system
...
for i=1:num_monte
    if  (K_total(i) >= 0) && (Os_mafic_monte_part(i) >= 0) && (Os_ultra_monte_part(i) >= 0)
        flux_times_ratio = @(t,y)flux_ht * (ratio_ht_norm - y) + flux_lt * (ratio_lt_norm - y) + ...
            ...
            K_total(i) * flux_river * (ratio_river_norm - y)+ Area_interp(t) * Os_mafic_monte_part(i) *...
            (dOs_mafic_monte(i) - y) + Area_interp(t) * Os_ultra_monte_part(i) * (dOs_ultra_monte(i) - y);
...
file_dir = strcat('norm_simple_', num2str(ratio_river), '_nomatch_high_rate_', num2str(t_interval / 1000), 'ky_', num2str(num_monte), 'monte');
...
csvwrite_with_headers('ddddd.csv', whole_data, headers)
